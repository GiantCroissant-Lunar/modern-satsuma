...
/// Extension methods for IGraph, for connected components operations.
public static class ConnectedComponentsExtensions
{
    /// Splits a graph into its connected components, returning each as a Subgraph view.
    /// This is a convenience method that combines ConnectedComponents with Subgraph creation.
    /// <param name="graph">The input graph.</param>
    /// <returns>An enumerable of Subgraph views, one for each connected component.</returns>
    public static IEnumerable<Subgraph> SplitIntoComponents(this IGraph graph)
    {
    ...
    /// Checks if the given set of nodes forms a connected subgraph.
    /// <param name="graph">The input graph.</param>
    /// <param name="nodes">The nodes to check for connectivity.</param>
    /// <returns>True if all nodes are reachable from each other within the subset.</returns>
    public static bool IsConnected(this IGraph graph, IEnumerable<Node> nodes)
    {
        var nodeSet = new HashSet<Node>(nodes);
        if (nodeSet.Count == 0) return true;
        if (nodeSet.Count == 1) return true;
        ...
        var components = new ConnectedComponents(subgraph);
